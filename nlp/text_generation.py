...
class RNN(nn.Module):
    ...
    def init_weights(self, batch_size):
        h = torch.zeros(self.num_rnn_layers, batch_size, self.hidden_size).to(device)
        c = torch.zeros(self.num_rnn_layers, batch_size, self.hidden_size).to(device)
        return h, c
    ...
    def train(self, length=250, iterations=5000, lr=0.003):
        ...
        for it in range(iterations):
            ...
            for idx in range(length):
                if idx > 0 and it > 100 and np.random.choice(1+(iterations - it)//500) == 0:
                    ...
                    in_char = input[idx].unsqueeze(0)
                out, (h, c) = self.rnn(in_char, h, c)
            ...
            loss_avg += loss.item()
            if it % 50 == 0 and it > 0:
                print(f'\nIteration {it} - average loss over the past 50 iterations: {np.round(loss_avg / 50, 2)}')
                loss_avg = 0
                print(self.generate())
